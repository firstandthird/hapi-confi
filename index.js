...
module.exports = (Hapi, options, allDone) => {
  ...
  const loadStrategery = (done, result) => {
    const server = result.server;
    const config = result.config;
  ...
  async.auto({
    ...
    log: ['server', loadLogging],
    auth: ['server', loadAuth],
    strategies: ['server', loadStrategery],
    plugins: ['server', loadPlugins],
    views: ['server', loadViews]
  }, (autoErr, result) => {
    if (autoErr) {
