...
lab.test('tests default with context', async() => {
  const server = await hapiconfi(Hapi, { configPath: `${__dirname}/conf`, config: { blah: 'hoover' } });
...
lab.test('tests multiple paths ', async() => {
  const server = await hapiconfi(Hapi, { configPath: [`${__dirname}/conf`, `${__dirname}/conf2`] });
...
lab.test('test prod ', async() => {
  const server = await hapiconfi(Hapi, { env: 'production', configPath: [`${__dirname}/conf`] });
...
lab.test('test yaml', async() => {
  const server = await hapiconfi(Hapi, { env: 'yaml', configPath: [`${__dirname}/conf`] });
  code.expect(server.settings.app.analytics.enabled).to.equal(true);
...
lab.test('returns error if it cannot parse any config file ', async() => {
  try {
    const server = await hapiconfi(Hapi, { configPath: [`${__dirname}/conf`, `${__dirname}/dysfunctional`] });
  } catch (err) {
